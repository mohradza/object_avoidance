...
def draw_optic_flow_field(gray_image, points, flow):
    ...
    for i, point in enumerate(points):
        ...
        vx = flow[i][0,0]
        vy = flow[i][0,1]
        cv2.line(color_img, (x,y), (x, y+vy), color_red, linewidth) # draw a red line from the point with vector = [vx, vy]        
...
################################################################################
# Compute the Average Tangential Optic Flow 
def average_ring_flow(gamma_size, num_rings, flow):
...
class Optic_Flow_Calculator:
    def __init__(self):
        ...
        self.optic_flow_pub = rospy.Publisher("optic_flow", Float32MultiArray, queue_size=10)
    ...
    def image_callback(self,image):
        ...
        try: # if there is an image
            ...
            tang_OF = average_ring_flow(self.gamma_size, self.num_rings, flow)           
